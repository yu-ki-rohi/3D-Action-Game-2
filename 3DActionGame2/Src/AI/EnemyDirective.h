...
namespace EnemyAI
{
	...
	public:
		static unsigned char MakeDirective(int direction_, CombatRange range_, Actions action_);
		static unsigned char MakeDirective(DirectiveInfo directive_info_);
	...
	public:
		...
		float GetMaximumDistance() const override;

		// TODO: IEnemyDirectiveWriterに純粋仮想関数として宣言
		void SetDirective(int id_, unsigned char directive_);
		void SetDirective(int id_, int direction_, CombatRange range_, Actions action_);
		void SetDirective(int id_, DirectiveInfo directive_info_);
		void SetDirective(int id_, int direction_);
	...
	private:
		...
		std::shared_ptr<const Transform> transformOfRepresentativeEnemy;
	};
