...
std::shared_ptr<CameraBase> ObjectFactory::CreateCameraTPS()
{
	...
	obj->SetTransform(std::make_shared<Transform>(Vector3(0.0f, 10.0f, 0.0f)));
...
std::shared_ptr<Player> ObjectFactory::CreatePlayer(std::shared_ptr <PlayerEventNotifier> notifier_, std::shared_ptr<CameraManager> camera_manager_)
{
	...
	obj->SetComponent(std::make_shared<PlayerStatus>(notifier_));
...
std::shared_ptr<Enemy> ObjectFactory::CreateEnemy()
{
	...
	obj->SetComponent(std::make_shared<Transform>(Vector3(20.0f, 0.0f, 10.0f), Vector3(0.15f, 0.15f, 0.15f)));
	obj->SetComponent(std::make_shared<Renderer>(
